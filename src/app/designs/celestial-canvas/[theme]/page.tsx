...
import { CELESTIAL_CANVAS_DEMO } from "@/data/demo-orders";

const DESIGN_SLUG = "celestial-canvas";
...
export default async function CelestialCanvasThemePage({ params }: Props) {
  const { theme } = await params;
  if (!isValidTheme(DESIGN_SLUG, theme)) notFound();
  return <CelestialCanvasCard order={CELESTIAL_CANVAS_DEMO} />;
}

...
import { NORDIC_MINIMAL_DEMO } from "@/data/demo-orders";

const DESIGN_SLUG = "nordic-minimal";
...
export default async function NordicMinimalThemePage({ params }: Props) {
  const { theme } = await params;
  if (!isValidTheme(DESIGN_SLUG, theme)) notFound();
  return <NordicMinimalCard order={NORDIC_MINIMAL_DEMO} />;
}

...
import { MUGHAL_MINIATURE_DEMO } from "@/data/demo-orders";

const DESIGN_SLUG = "mughal-miniature";
...
export default async function MughalMiniatureThemePage({ params }: Props) {
  const { theme } = await params;
  if (!isValidTheme(DESIGN_SLUG, theme)) notFound();
  return <MughalMiniatureCard order={MUGHAL_MINIATURE_DEMO} />;
}

...
import { MONSOON_GARDEN_DEMO } from "@/data/demo-orders";

const DESIGN_SLUG = "monsoon-garden";
...
export default async function MonsoonGardenThemePage({ params }: Props) {
  const { theme } = await params;
  if (!isValidTheme(DESIGN_SLUG, theme)) notFound();
  return <MonsoonGardenCard order={MONSOON_GARDEN_DEMO} />;
}
